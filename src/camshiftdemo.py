...
def on_mouse( event, x, y, flags, param ):
    ...
    if (event == 4): # Left mouse button up
        select_object = 0
...
if __name__ == '__main__':
    ...
    while 1:
        ...
        pt1 = cv.cvPoint (selection.x, selection.y)
        pt2 = cv.cvPoint(selection.x+selection.width,selection.y+selection.height)
        color = hsv2rgb (50)
        cv.cvRectangle (frame, pt1, pt2, color, 3, 4, 0)

        cv.cvCalcBackProject( hue , backproject, hist );
        cv.cvAnd( backproject, mask, backproject, None );
        cv.cvCamShift( backproject, track_window,
                    cvTermCriteria( CV_TERMCRIT_EPS | CV_TERMCRIT_ITER, 10, 1 ),
                    track_comp, track_box );
        track_window = track_comp.rect;

        cv.cvEllipseBox( image, track_box, CV_RGB(255,0,0), 3, CV_AA, 0 );
